...
const createProjectCard = (project) => {
    ...
    if (project.deployment) {
        ...
        const deploymentLabel =
            project.deploymentLabel || (project.visibility === "private" ? "Production Site" : "Live Demo");
        links.appendChild(
            createActionLink(
                deploymentType,
                project.deployment,
    ...
    if (project.source) {
        const sourceLabel = project.source.includes("README.md") ? "README" : "Source Code";
        links.appendChild(
            createActionLink("code", project.source, sourceLabel)
        );
    } else if (project.visibility === "private") {
        links.appendChild(createActionInfo("private", "Personal prod"));
    }
...
const freelanceProjectGrid = document.querySelector("[data-freelance-project-grid]");

const renderFreelanceProjects = async () => {
    if (!freelanceProjectGrid) return;
    ...
    try {
        ...
        freelanceProjectGrid.innerHTML = "";
        projects.forEach((project) => {
            const card = createProjectCard(project);
            freelanceProjectGrid.appendChild(card);
        });
        hydrateHeatText();
    } catch (error) {
        const message = document.createElement("p");
        message.className = "projects__error";
        message.textContent =
            "Unable to load freelance projects right now. Please refresh or try again later.";
        freelanceProjectGrid.appendChild(message);
        console.error(error);
    }
};
